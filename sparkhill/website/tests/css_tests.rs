...
fn test_reset_css_has_dark_mode_support() {
    let css_content = fs::read_to_string("static/css/reset.css").expect("Failed to read reset.css");
...
fn test_reset_css_structure() {
    ...
    assert!(
        css_content.contains("box-sizing: border-box"),
        "Should have box-sizing reset"
    );
